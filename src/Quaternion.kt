...
data class Quaternion(val scalar: Float, val axis: Vector3){
    ...
    fun length() = Math.sqrt( (scalar * scalar + axis.x * axis.x + axis.y * axis.y + axis.z * axis.z).toDouble()).toFloat()
    fun normal() = length().let { Quaternion(scalar = scalar / it, axis = axis / it) }
    //fun inverse() = conjugate() / normal()
    fun transformation(): Matrix4{
        val w2 = scalar * scalar
        val x2 = axis.x * axis.x
        val y2 = axis.y * axis.y
        val z2 = axis.z * axis.z
        return Matrix4(
                a = w2 + x2 - y2 - z2, b = 2*axis.x*axis.y - 2*scalar*axis.z, c = 2*axis.x*axis.z + 2 * scalar*axis.y, d = 0f,
                e = 2*axis.x*axis.y + 2*scalar*axis.z, f = w2 - x2 + y2 - z2, g = 2*axis.y*axis.z + 2 * scalar*axis.x, h = 0f,
                i = 2*axis.x*axis.z - 2 * scalar*axis.y, j = 2*axis.y*axis.z - 2 * scalar*axis.x, k = w2 - x2 - y2 + z2, l = 0f,
                m = 0f, n = 0f, o = 0f, p = 1f
        )
    }
    companion object {
        @JsName("from") fun from(angle: Float, axis: Vector3) = Quaternion(
                scalar = Math.cos(angle/2.0).toFloat(),
                axis = axis * Math.sin(angle/2.0).toFloat()
        )
    }
}